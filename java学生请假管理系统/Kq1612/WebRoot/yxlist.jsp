 ...
 <script type="text/javascript">
         ...
         function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/YxServlet?mode=list&pageNoStr=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/YxServlet?mode=list&pageNoStr="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/YxServlet?mode=list&pageNoStr="+totlePage;
	            submit();
	        }
		}
		...
		{ 
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/YxServlet?mode=list";
	            submit();
	        }
		}
	
		function modify(id){
			window.location.href="<%=path %>/YxServlet?mode=modifybefore&id="+id;
		}
		function del(id){
			window.location.href="<%=path %>/YxServlet?mode=deletes&id="+id;
		}
		function tp(id){
			window.location.href="<%=path %>/YxServlet?mode=tplist&id="+id;
		}
		function ly(id){
       ...
       <table class="table table-bordered">
         <tr>
          ...
          <%for(int i=0;i<list.size();i++){ 
        	Yx bean = (Yx)list.get(i);
        %>
         <tr>
           <td align="center"><%=bean.getId() %></td>
           <td align="center"><%=bean.getNames() %></td>
		   <td>
		   <a id="DataGrid1_ctl08_LinkButton1" href="<%=path %>/YxServlet?mode=deletes&id=<%=bean.getId() %>">删除</a></td>
         </tr>
         <%} %>
       </table>
   <table  class="margin-bottom-20 table  no-border" >
