 ...
 <script type="text/javascript">
         ...
         function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/QjServlet?mode=slist1&pageNoStr=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/QjServlet?mode=slist1&pageNoStr="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/QjServlet?mode=slist1&pageNoStr="+totlePage;
	            submit();
	        }
		}
		...
		{ 
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/QjServlet?mode=slist1";
	            submit();
	        }
		}
       ...
       <table class="table table-bordered">
        ...
        %>
         <tr>
		   ...
		   <td>
		   <a id="DataGrid1_ctl08_LinkButton1" href="<%=path %>/QjServlet?mode=sh1&id=<%=bean.getId() %>&states=1">通过</a>
		   <a id="DataGrid1_ctl08_LinkButton1" href="<%=path %>/QjServlet?mode=sh1&id=<%=bean.getId() %>&states=2">不通过</a>
		   </td>
         </tr>
         <%} %>
       </table>
