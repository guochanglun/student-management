 ...
 <script type="text/javascript">
         ...
         function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/StuServlet?mode=list&pageNoStr=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/StuServlet?mode=list&pageNoStr="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/StuServlet?mode=list&pageNoStr="+totlePage;
	            submit();
	        }
		}
		...
		{ 
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/StuServlet?mode=list";
	            submit();
	        }
		}
	
		function modify(id){
			window.location.href="<%=path %>/StuServlet?mode=modifybefore&id="+id;
		}
		function del(id){
			window.location.href="<%=path %>/StuServlet?mode=deletes&id="+id;
		}
		function tp(id){
			window.location.href="<%=path %>/StuServlet?mode=tplist&id="+id;
		}
		function ly(id){
...
学生管理
   ...
   <div class="title_right"><strong>学生清单</strong></div>  
    <form id="memberForm">
     ...
     <td width="23%"><input type="text" name="names" id="names" class="span1-1" /></td>
       ...
       <table class="table table-bordered">
         <tr>
           ...
           <td align="center" nowrap="nowrap" bgcolor="#f1f1f1"><strong>学号</strong></td>
