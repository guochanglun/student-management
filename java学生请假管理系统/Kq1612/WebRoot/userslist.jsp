 ...
 <script type="text/javascript">
         ...
         function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/UsersServlet?mode=list&pageNoStr=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/UsersServlet?mode=list&pageNoStr="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/UsersServlet?mode=list&pageNoStr="+totlePage;
	            submit();
	        }
		}
		...
		{ 
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/UsersServlet?mode=list";
	            submit();
	        }
		}
	
		function modify(id){
			window.location.href="<%=path %>/UsersServlet?mode=modifybefore&id="+id;
		}
		function del(id){
			window.location.href="<%=path %>/UsersServlet?mode=deletes&id="+id;
		}
		function tp(id){
			window.location.href="<%=path %>/UsersServlet?mode=tplist&id="+id;
		}
		function ly(id){
...
<ul class="breadcrumb">当前位置：
  用户管理
   ...
   <div class="title_right"><strong>用户清单</strong></div>  
    <form id="memberForm">
     ...
     <td width="23%"><input type="text" name="names" id="names" class="span1-1" /></td>
       ...
       <table class="table table-bordered">
         <tr>
           ...
           <td align="center" nowrap="nowrap" bgcolor="#f1f1f1"><strong>用户名</strong></td>
        ...
        %>
         <tr>
		   ...
		   <td>
		   <a id="DataGrid1_ctl08_LinkButton1" href="<%=path %>/UsersServlet?mode=deletes&id=<%=bean.getId() %>">删除</a></td>
         </tr>
         <%} %>
       </table>
   <table  class="margin-bottom-20 table  no-border" >
